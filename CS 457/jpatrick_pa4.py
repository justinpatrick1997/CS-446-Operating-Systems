...
def selectStar(qb, qbPD):
    try:
        correctDB()
        myTB = re.split("FROM ", qb, flags=re.IGNORECASE)[1].lower()
        if "WHERE" in qbPD:
            myTB = re.split("WHERE", myTB, flags=re.IGNORECASE)[0]
        if " " in myTB:
            myTB = myTB.split(" ")[0]
        myFile = os.path.join(wrkDir, myTB)
        output = ""
        if os.path.isfile(myFile):
            with open(myFile, "r+") as tb:
                if "WHERE" in qbPD:
                    findMe = re.split("WHERE ", input, flags=re.IGNORECASE)[1]
                    data = tb.readlines()
                    mainCount, output = where(findMe, "select", data)
                    for line in output:
                        print line
                if "SELECT *" in qbPD:
                    if not output == "":
                    ...
                    if not output == "":  # Checks if the result is allocated
                        lines = ouput
                    else:
            ...
            print "!Failed to query table " + myTB + " because it does not exist"
    ...
    try:
        ...
        while True:
            ...
            if "UPDATE" in inputStr.upper():
                ...
                lkArr[0] = myFile
                myTB = myTB[0].upper() + myTB[1:]
                path = "./CS457_PA4/" + myTB
                update(path, *stripper(cmd))
                lkFlag = 1
                

        #print path
        if myFile in fileArr:
            print "Error: table", myTB, "is locked."
            return ["table"], 0
        return lkArr, lkFlag
...
def update(path, indexCol, indexVal, valueCol, replaceVal):
    tbFile = open(path, "r")
    lines = tbFile.read().split("\n")
    cols = lines.pop(0)
    newDT = [cols]
    colName = list(map(lambda x: x.split()[0], cols.split(" | ")))
    idx_col = colName.index(indexCol)
    val_col = colName.index(valueCol)

    for line in lines:
        arr = list(map(int, line.split(" | ")))
        if arr[idx_col] == indexCol:
            arr[val_col] = replaceVal
        arr = list(map(str,arr))
        newDT.append(" | ".join(arr))
    newDT = "\n".join(newDT)
...
def stripper(str_):
    strArr = str_.split()
    setIdx = strArr.index('set')
    whereIdx = strArr.index('where')
    setData = strArr[setIdx + 1:setIdx + 4]
    whereData = strArr[whereIdx + 1:whereIdx + 4]
    return whereData[0], whereData[-1], setData[0], setData[-1]
